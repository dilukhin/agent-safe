...
class BuildIdentityTests(unittest.TestCase):
    ...
    def test_packaged_runtime_keeps_full_sha_without_git_checkout(self):
        artifact = ROOT / "dist" / "safe.pyz"
        build_env = os.environ.copy()
        build_env["AGENT_SAFE_SOURCE_COMMIT"] = SOURCE_SHA
        build = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "build_zipapp.py")],
            cwd=ROOT,
            env=build_env,
            encoding="utf-8",
            capture_output=True,
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                runtime_cwd = Path(td)
                runtime_artifact = runtime_cwd / "safe.pyz"
                shutil.copy2(artifact, runtime_artifact)

                runtime_env = os.environ.copy()
                runtime_env.pop("AGENT_SAFE_SOURCE_COMMIT", None)
                runtime_env["PYTHONPATH"] = str(runtime_artifact)
                ...
                version = subprocess.run(
                    [sys.executable, str(runtime_artifact), "--version"],
                    cwd=runtime_cwd,
                    env=runtime_env,
                    encoding="utf-8",
                    capture_output=True,
