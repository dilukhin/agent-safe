...
Выполнить рискованную команду только с явным target, expected state, rollback (либо планом восстановления) и verify:
...
Для `exec-risky` без обратной команды можно вместо `--rollback-command` передать
`--recovery-contract-file recovery.json`. Это явный режим контрольной точки:
до запуска проверяются точная цель, условия, срок действия и SHA-256 локального
свидетельства, а план фиксируется в журнале. Источник восстановления выбирается явно.
Формат, пример и границы доверия: [контракт восстановления](docs/RECOVERY_CONTRACT.md).

Для локального действия доступен третий, взаимоисключающий вариант:
`--rollback-plan-file`. Он сохраняет объявленные файлы отката до запуска действия.
Отдельная команда `recover` проверяет комплект, выполняет согласованное восстановление
и его проверку, сохраняя исходную блокировку. Первый профиль: одна обычная файловая
цель и сохранённые Python-скрипты. Формат и ограничения:
[сохранённый локальный откат](docs/SAVED_ROLLBACK.md).
...
4. определить точные цели и способ проверяемого восстановления;
5. подготовить отдельный план ручного или специализированного восстановления;
...
Обычные `undo` и `redo` при действующей блокировке запрещены. Не следует снимать
блокировку только ради их запуска: сначала требуется согласованное восстановление
и проверка результата. Отдельный исполнитель recovery для shell-действий пока
не реализован.

Вне Recovery Mode файловые `undo/redo` сверяют состояние журнала, идентичность
объекта и родителя назначения, отвергают ссылки/reparse point и проверяют результат.
Для обычного файла дополнительно сравнивается SHA-256; каталог переносится как
объект без обещания рекурсивной проверки неизменности всех потомков. Старые записи
без достаточного evidence читаются, но не дают права автоматически переместить
объект. Ошибка после начала восстановления сохраняет блокировку и сведения о ходе.

Снять блокировку можно только после ручного анализа и подтверждённого восстановления:
...
## План развития и результаты аудита

- [Текущий план работ](docs/WORK_PLAN.md) — очередь задач, зависимости и критерии приёмки.
- [Аудит восстановления и SSH-проверок от 19.09.2026](docs/audits/2026-09-19-stage0.md) — воспроизведения и подтверждённые пробелы, учтённые в #26 и #27.
- [Проект структурированных вызовов и самодостаточного отката](docs/PROCESS_RECOVERY_DESIGN.md) — предложение для согласования #15/#25A; описанные новые интерфейсы ещё не реализованы.
- [Интеграционная проверка SSH-адаптера](docs/SSH_RELAY_INTEGRATION_CHECKLIST.md) — подготовленный сценарий, отдельно от модульных тестов.
