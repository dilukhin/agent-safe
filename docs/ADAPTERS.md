...
## Идентичность SSH-проверки

Для машинного ответа после возможного запуска `ssh-relay-risky` требует непустые
строковые `remote_host`/`remote_user` и целочисленный порт 1–65535 (не boolean).
Verify должен сообщать тот же host/port/user, что основное действие; mode и явно
указанное имя сессии также проверяются. Несовпадение или неполный ответ не дают
`done`: включается Recovery Mode, исходная команда не повторяется.

При доказанном `not_started` endpoint может отсутствовать: это не превращает
незапущенную операцию в выполненную. Ошибка receipt/unknown также не становится
успехом после удачного verify.

Это проверка согласованности ответов доверенного relay, не криптографическое
подтверждение хоста. `host_label` остаётся меткой: фактическую привязку сессии,
SSH host identity и точную тестовую цель нужно проверить до изменения состояния.
Имена хостов сравниваются точно, без автоматического приравнивания DNS-алиасов.
Реальный интеграционный прогон двух сессий остаётся отдельной частью #27.
